...
export const Route = createFileRoute("/apply/student")({
    ...
    <main className="min-h-screen bg-surface py-16 px-6">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <span className="mt-6 inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-xs text-primary">
          <span className="h-1 w-1 rounded-full bg-primary" /> Join as a student
        </span>
        <h1 className="mt-5 font-display text-[clamp(2rem,4vw,3.25rem)] leading-[1.05] tracking-[-0.03em]">
          Start learning <span className="italic text-gradient-primary">music.</span>
        </h1>
        <p className="mx-auto mt-4 max-w-lg text-sm text-foreground/70">
          Tell us a bit about yourself. Once your application is approved, we&apos;ll email you an
          invite to set your password and access the platform.
        </p>
      </div>
      <div className="mt-10">
        <ApplyForm role="student" />
      </div>
    </main>
  );
}
