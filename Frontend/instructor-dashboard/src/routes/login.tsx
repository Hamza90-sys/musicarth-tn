  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (payload.user.role !== REQUIRED_ROLE) {
        setError("This account is not allowed to access the instructor dashboard.");
        return;
      }
      ...
      <div className="w-full max-w-md">
        <div className="mb-8 flex flex-col items-center gap-3 text-center">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
            <Presentation className="h-3.5 w-3.5" /> Instructor
          </span>
        </div>
        ...
        <Card className="border-border/60 bg-card/80 shadow-[0_24px_70px_-20px_rgba(80,40,160,0.35)] backdrop-blur">
          <CardHeader className="space-y-1 text-center">
            ...
            <CardDescription>Sign in to manage your courses and sessions.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <p className="text-center text-sm text-muted-foreground">
              ...
              <a
                href="http://localhost:5173/apply/instructor"
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          © 2026 Musicarth · Teach music, beautifully.
        </p>
      </div>
    </div>
  );
