...
/**
 * Embeds a Daily.co WebRTC room (spec 11). The room URL + meeting token come
 * from the backend's GET /sessions/:id/room (time-gated). The iframe is torn
 * down on unmount so only one Daily instance ever exists at a time.
 */
export function DailyRoom({
  ...
  useEffect(() => {
    ...
    // Mount once per room; roomUrl/token are stable for the session lifetime.
